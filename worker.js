/*
 * https://github.com/PlentyGram/Cloudflare-CORS-Proxy
 *
...
*/
const config = {
    // 从 https://www.plentygram.com/ 申请并修改令牌
    plentygramToken: "00000000-0000-0000-0000-000000000000",
...
async function handleRequest(event) {
    ...
    try {
        ...
        if (request.method == "OPTIONS" || url.length < 3 || url.indexOf('.') == -1 || url == "favicon.ico" || url == "robots.txt") {
            ...
            outBody = JSON.stringify({
                code: invalid ? 400 : 0,
                usage: 'Host/{URL}',
                source: 'https://github.com/PlentyGram/Cloudflare-CORS-Proxy',
    ...
    //日志接口
    if (config.plentygramToken != "00000000-0000-0000-0000-000000000000") {
        plentygram.add(event, request, response);
    }
...
const plentygram = {
    ...
    add: (event, request, response) => {
        let url = `https://logsene-receiver.plentygram.com/${config.plentygramToken}/example/`;
        const body = plentygram.buildBody(request, response);
